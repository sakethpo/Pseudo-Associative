	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  if (cp->hsize)
  ...
  else
    {
      ...
      if(cp->pseudo==1)
      {
		md_addr_t elongatedTag = CACHE_TAG_PSEUDOASSOC(cp, addr);	  
		
	      for (blk=cp->sets[set].way_head;
		   blk;
		   blk=blk->way_next)
		{
		    if (blk->tag == elongatedTag && (blk->status & CACHE_BLK_VALID))
		    {	
		    	goto cache_hit;
		    }
		}
    ...
    if(cp->pseudo==1)
    {
	    ...
	    else
	    {
			   ...
			   blk=blk->way_next)
		    {
			  if (blk->tag == elongatedTag && (blk->status & CACHE_BLK_VALID))
			  {
				exchange = cp->sets[newSet];
				cp->sets[newSet] = cp->sets[set];
				cp->sets[set] = exchange;		   
